...
class testBase(unittest.TestCase):
    def test_base_id(self):
        ...
        self.assertEqual(bs3.id, 9)
        self.assertEqual(bs5.id, 4)
        self.assertEqual(bs6.id, 5)
        self.assertEqual(bs7.id, 7)
        self.assertEqual(bs8.id, 7)
        self.assertEqual(bs9.id, 5)
        self.assertEqual(bs0.id, 9)
    ...
    def test4_json_to_file4(self):
        ...
        with open("Square.json", "r") as file:
            file1 = file.read()
            list_output = Square.from_json_string(file1)
            self.assertEqual(list_output, new_list)
    ...
    def test5_json_to_file5(self):
        ...
        with self.assertRaises(AttributeError):
            new_string = "string"
            Square.save_to_file(new_string)
    ...
    def test6_json_to_file6(self):
        ...
        with self.assertRaises(TypeError):
            fav_num = 13
            Square.save_to_file(fav_num)

    def test7_json_to_file7(self):
        """ test json string into file"""
        Base._Base__nb_objects = 0
        with self.assertRaises(AttributeError):
            sec_list = ["Leonie", "cool", "dad"]
            Square.save_to_file(sec_list)
    ...
    def test_json_to_file10(self):
        ...
        s1 = Square(89, 12, 24, 5)
        s2 = Square(4, 5, 7, 9)
        jump = [s1, s2]
        Square.save_to_file(jump)
        with open("Square.json", "r") as my_file:
            jump = [s1.to_dictionary(), s2.to_dictionary()]
            self.assertEqual(json.dumps(jump), my_file.read())
